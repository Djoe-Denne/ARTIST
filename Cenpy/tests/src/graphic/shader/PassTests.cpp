...
{
    // Arrange
    auto vertexShader = std::make_shared<mock_shader::MockShader>("vertex_shader_path", context::ShaderType::VERTEX);
    auto fragmentShader = std::make_shared<mock_shader::MockShader>("fragment_shader_path", context::ShaderType::FRAGMENT);

    // Act and Assert
    EXPECT_NO_THROW(shader::Pass<api::MockOpenGL> pass({vertexShader, fragmentShader}));
}
...
TEST_F(PassTest, attachShaders)
{
    ...
    // Act
    shader::Pass<api::MockOpenGL> pass({vertexShader, fragmentShader});
...
TEST_F(PassTest, DeleteMustFree)
{
    ...
    // Act
    const auto &pass = new shader::Pass<api::MockOpenGL>({vertexShader, fragmentShader});
...
{
    // Arrange
    auto failingShader = std::make_shared<mock_shader::MockShader>("vertex_shader_path", context::ShaderType::VERTEX);
    EXPECT_CALL(*failingShader, load()).WillOnce(::testing::Throw(std::runtime_error("Shader load failed")));

    // Act and Assert
    EXPECT_THROW(shader::Pass<api::MockOpenGL> pass({failingShader}), std::runtime_error);
}
