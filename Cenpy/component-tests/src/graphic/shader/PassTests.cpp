...
{
protected:
    std::shared_ptr<pipeline::Shader<api::OpenGL>> vertexShader;
    std::shared_ptr<pipeline::Shader<api::OpenGL>> fragmentShader;

    void SetUp() override
    {
        OpenGLComponentTest::SetUp();
        vertexShader = std::make_shared<pipeline::Shader<api::OpenGL>>("test-datas/shaders/vertex/good/minimal.vert", context::ShaderType::VERTEX);
        fragmentShader = std::make_shared<pipeline::Shader<api::OpenGL>>("test-datas/shaders/fragment/good/minimal.frag", context::ShaderType::FRAGMENT);
    }
};

TEST_F(PassTest, PassCreationTest)
{
    ASSERT_NO_THROW(pipeline::Pass<api::OpenGL>({vertexShader, fragmentShader}));
}

TEST_F(PassTest, UniformManagementTest)
{
    pipeline::Pass<api::OpenGL> pass({vertexShader, fragmentShader});
    pass.load();
...
{
    ASSERT_NO_THROW(pipeline::Pass<api::OpenGL> pass({vertexShader, fragmentShader}));
...
TEST_F(PassTest, FreeResourcesTest)
{
    ...
    auto pass = new pipeline::Pass<api::OpenGL>({vertexShader, fragmentShader});
